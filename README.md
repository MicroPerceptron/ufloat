...
Same-layout UF8 exponentiation is also available and uses generated lookup
tables:

```rust
use unsigned_float::{PowUf, Uf8};
...
`0 <= u <= 1` case. Same-layout UF8 complement powers use generated lookup
tables, so `u` and `a` can stay compact from input through output.
...
`Add`, `Sub`, `Mul`, `Div`, same-layout `PowUf`, and same-layout `Pow1mUf`.
These tables are generated by `build.rs` into Cargo's `OUT_DIR` and embedded
with `include_bytes!`, so they do not need to be checked in.
...
- `Pow1mUf` for native and same-layout UF8 complement powers
